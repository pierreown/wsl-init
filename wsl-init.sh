...
enable_wsl_init() {
    ...
    ini '/etc/wsl.conf' 'boot' 'command' '/opt/wsl-init/wsl-init-boot.sh'
    fmt SUC "Modified /etc/wsl.conf, saved old file to *.old"

    rm -f /etc/profile.d/000-wsl-init-enter.sh
    cp /opt/wsl-init/wsl-init-enter.profile.sh /etc/profile.d/000-wsl-init-enter.sh
    fmt SUC "Created /etc/profile.d/000-wsl-init-enter.sh"
}
...
disable_wsl_init() {
    ...
    rm -f /etc/profile.d/000-wsl-init-enter.sh
    fmt SUC "Removed /etc/profile.d/000-wsl-init-enter.sh"
}
